...
Wenn das Modul die eigene Initialisierung abgeschlossen hat, meldet es die Bereitschaft an den Host. Im Verona-Kontext liefert das Modul seine Metadaten mit, d. h. der Host kann prüfen, ob das richtige Modul geladen wurde bzw. ob bestimmte Anpassungen in der Kommunikation erfolgen müssen.
...
Bei schnellen Tasteneingaben oder ambitionierter Beobachtung des Userverhaltens fallen sehr viele Daten an. Beim Player-Modul kann man die Daten aufteilen (sog. `dataParts`), aber auch sonst könnte es viel werden. Die Hostanwendung sollte einen Puffer einrichten und nicht alles sofort wegspeichern. Bei `rxjs` gibt es beispielsweise den Operator `debounceTime`.
:::

# Beenden: unnötig

Da Änderungen sofort gemeldet werden, ist ein formelles Beenden des Moduls unnötig. Es gibt bei keinem Verona-Modul Bedarf, vor Entfernen des Moduls bestimmte Funktionen aufzurufen. Wenn das Modul nicht mehr benötigt wird (oben dargestellt als Navigation), kann das `<iframe>`-Element entfernt oder geleert werden (`scrdoc=""`).
