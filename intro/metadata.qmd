...
* `type`: Die möglichen Werte dieser Aufzählung sind `PLAYER`, `EDITOR`, `SCHEMER` und `WIDGET`. Bitte lesen die entsprechenden Teile dieser Dokumentation für ein besseres Verständnis.
* `model`: Hier kann man den Typ näher spezifizieren. Bei einem Player könnte man beispielsweise die Id und Version des Datenformates angeben, das der Player maximal verarbeiten kann. Bei einem Widget muss hier der Untertyp angegeben werden, also z. B. `PERIODIC_TABLE` oder `CALC`.
